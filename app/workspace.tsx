      ...
      {(reminderJob || directReminderOpen) && (
        <div className="modal-backdrop" onClick={closeReminderEditor}>
          <section className="reminder-sheet" role="dialog" aria-modal="true" aria-labelledby="reminder-title" onClick={e=>e.stopPropagation()}>
            ...
            <div className="reminder-block"><strong>Time</strong><div className="reminder-chips"><button type="button" className={reminderTime==="09:00"?"selected":""} onClick={()=>setReminderTime("09:00")}><span className="chip-check">✓</span>Morning</button><button type="button" className={reminderTime==="15:00"?"selected":""} onClick={()=>setReminderTime("15:00")}><span className="chip-check">✓</span>Afternoon</button><button type="button" className={reminderTime==="19:00"?"selected":""} onClick={()=>setReminderTime("19:00")}><span className="chip-check">✓</span>Evening</button><label className={!["09:00","15:00","19:00"].includes(reminderTime) ? "date-chip selected reminder-time-chip" : "date-chip reminder-time-chip"}><Icon name="clock" size={16}/><input aria-label="Pick reminder time" type="time" value={reminderTime} onChange={e=>setReminderTime(e.target.value)}/><span className="reminder-time-period">{Number(reminderTime.slice(0,2))>=12?"PM":"AM"}</span></label></div></div>
