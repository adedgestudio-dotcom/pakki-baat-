...
export default function AdminPage(){
  ...
  if(state==="blocked") return <main className="admin-shell"><div className="admin-access-card"><span>OWNER ONLY</span><h1>Admin access</h1><p>{email} is the account currently open in Pakki Baat. It has not been changed.</p><p className="admin-login-note">Your testing account will stay signed in. Open the owner login in a separate private window so it uses its own browser session.</p><button type="button" onClick={()=>{window.localStorage.setItem("pakki-admin-intent","1");window.location.href="/admin/login";}}>Login as owner</button><a className="admin-button" href="/">Back to Pakki Baat</a></div></main>;
