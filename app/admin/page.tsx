...
export default function AdminPage(){
  ...
  if(!unlocked) return <main className="admin-shell"><form className="admin-access-card" onSubmit={login}>
    ...
    <div className="admin-pin-wrap"><input className="admin-pin" type={showPin?"text":"password"} inputMode="numeric" autoComplete="current-password" value={pin} onChange={e=>setPin(e.target.value)} placeholder="Enter PIN" maxLength={12} autoFocus /><button className="admin-pin-eye" type="button" aria-label={showPin?"Hide PIN":"Show PIN"} onClick={()=>setShowPin(v=>!v)}><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12Z"/><circle cx="12" cy="12" r="2.5"/>{showPin&&<path d="M4 4l16 16"/>}</svg></button></div>
