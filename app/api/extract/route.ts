...
export async function POST(req:NextRequest){
 ...
 const form=await req.formData();const file=form.get("file"),text=String(form.get("text")||"").slice(0,6000);const date=String(form.get("today")||""),mode=String(form.get("mode")||"extract");
 if(mode!=="extract"&&mode!=="transcribe")return Response.json({error:"Invalid capture mode."},{status:400});
 if(mode==="transcribe"&&(!(file instanceof File)||!file.type.startsWith("audio/")))return Response.json({error:"Choose a voice note to transcribe."},{status:400});
 if(file instanceof File&&(file.size>2_000_000||(!["image/png","image/jpeg","image/webp","audio/mpeg","audio/mp4","audio/webm","audio/ogg","audio/wav","audio/x-wav","video/webm"].includes(file.type))))return Response.json({error:"Use a PNG, JPG, WebP screenshot or MP3, M4A, WAV, WebM recording under 2 MB."},{status:400});
 ...
 const trans=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`},body:audio,signal:AbortSignal.timeout(25000)});if(!trans.ok)throw new Error("The recording could not be transcribed. Try a shorter recording or type the details.");transcript=String((await trans.json()).text||"").trim();
 if(mode==="transcribe")return transcript?Response.json({text:transcript.slice(0,6000)}):Response.json({error:"No speech was detected. Try recording again."},{status:422});
 }
