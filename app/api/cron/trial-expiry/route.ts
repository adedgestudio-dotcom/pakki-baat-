...
function cfg(){return{base:process.env.NEXT_PUBLIC_SUPABASE_URL||"",service:process.env.SUPABASE_SERVICE_ROLE_KEY||"",resend:process.env.RESEND_API_KEY||"",from:process.env.TRIAL_EMAIL_FROM||"Pakki Baat <onboarding@resend.dev>"}}
...
export async function GET(req:NextRequest){
  ...
  const{base,service,resend,from}=cfg();
  if(!base||!service)return NextResponse.json({error:"Supabase server configuration missing"},{status:503});
  if(!resend)return NextResponse.json({ok:false,error:"RESEND_API_KEY is not configured",sent:0},{status:503});
  try{
    const now=new Date();
