...
AI requests require a valid Supabase login and use an atomic database quota of 30 attempted captures per account per database day. Failed processing attempts also count. Uploads are limited to 2 MB; microphone recordings stop after one minute. The server uses OpenAI transcription and structured extraction, then opens an editable draft. Uploaded media is forwarded for processing and is not stored in Supabase; the extracted text is stored when the user saves the commitment.
